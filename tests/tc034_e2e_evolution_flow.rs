...
fn test_e2e_evolution_flow_with_custom_template() {
    ...
    // 言語ディレクトリ (ja/) の中にダミーファイルを作成
    let lang_dir = custom_template.join("ja");
    ...
    let required_files = [
        ...
        "docs/adr/index.md",
        "docs/adr/.gitkeep",
        "docs/design-docs/DESIGN_TEMPLATE.md",
    ...
    for rel_path in &required_files {
        let path = lang_dir.join(rel_path);
        fs::create_dir_all(path.parent().unwrap()).unwrap();
        fs::write(&path, "dummy content").unwrap();
    }

    // RULES.md だけ進化させる
    let rules_template_path = lang_dir.join("docs/RULES.md");
