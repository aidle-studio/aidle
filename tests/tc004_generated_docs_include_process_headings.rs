use assert_cmd::cargo::cargo_bin_cmd;
use std::fs;
use tempfile::tempdir;

#[test]
fn tc004_generated_docs_include_process_and_agreement_headings() {
    let temp = tempdir().expect("failed to create temp directory");
    let root = temp.path();

    cargo_bin_cmd!("aidle")
        .current_dir(root)
        .arg("init")
        .assert()
        .success();

    let prd = fs::read_to_string(root.join("PRD.md")).expect("failed to read PRD.md");
    let rules = fs::read_to_string(root.join("docs/RULES.md")).expect("failed to read RULES.md");
    let test_plan =
        fs::read_to_string(root.join("docs/TEST_PLAN.md")).expect("failed to read TEST_PLAN.md");

    assert!(
        prd.contains("Vision & Goals"),
        "PRD.md must include Vision & Goals: {prd}"
    );
    assert!(
        prd.contains("Core Features"),
        "PRD.md must include Core Features section: {prd}"
    );
    assert!(
        rules.contains("Agreement Gate"),
        "RULES.md must mention agreement gate: {rules}"
    );
    assert!(rules.contains("TDD"), "RULES.md must contain TDD: {rules}");
    assert!(
        test_plan.to_lowercase().contains("traceability") || test_plan.contains("トレーサビリティ"),
        "TEST_PLAN.md must contain traceability matrix: {test_plan}"
    );
}
