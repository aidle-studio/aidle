...
fn tc030_no_args_shows_help_and_exits_with_code_2() {
    ...
    assert_eq!(
        output.status.code(),
        Some(2),
        "exit code must be 2 for missing subcommand"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("Usage:"), "stderr must contain usage info");
    assert!(
        stderr.contains("init [dir]"),
        "stderr must contain init command info"
    );
}
...
fn tc030_help_flag_shows_help_and_exits_with_code_0() {
    for flag in ["-h", "--help"] {
        ...
        assert_eq!(
            output.status.code(),
            Some(0),
            "exit code must be 0 for help with {flag}"
        );
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(
            stdout.contains("Usage:"),
            "stdout must contain usage info with {flag}"
        );
    }
}
...
fn tc030_init_help_flag_shows_help_and_exits_with_code_0() {
    ...
    assert_eq!(
        output.status.code(),
        Some(0),
        "exit code must be 0 for init --help"
    );
    let stdout = String::from_utf8_lossy(&output.stdout);
