...
### MS8: テンプレート基盤の完成（計画待ち）
**達成条件**
- `default` テンプレートが運用開始に十分な品質・粒度で完成している。
- 追加テンプレート展開を見据えた共通構造（可変項目/固定項目）が整理されている。

**TODO**
- [ ] `templates/default/` の各ファイルを完成版として見直す（内容不足・重複・矛盾の解消）
- [ ] テンプレート内の可変項目を明確化し、将来テンプレート追加の差分点を整理する
- [ ] テンプレート品質の契約テスト（必須見出し/参照整合）を追加する
...
**TODO**
- [ ] 不足情報の判定ルールを仕様化する（何を必須入力とするか）
- [ ] 対話プロンプト実装（入力検証・再入力・キャンセル）を追加する
- [ ] `--non-interactive` / TTY自動判定との整合テストを追加する
