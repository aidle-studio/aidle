...
| TC-022 | AC-011 | integration | 適応レイヤ生成を有効化した場合、GC-2対象ファイルが生成される | Red |
| TC-023 | AC-012 | integration | 適応レイヤ生成が無効（既定）な場合、GC-2対象ファイルは生成されない | Red |
...
- 必須フロー（`TC-001`〜`TC-021`）に対応する統合テストは、常に実行対象とする。
